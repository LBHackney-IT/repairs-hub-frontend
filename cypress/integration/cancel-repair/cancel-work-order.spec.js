...
describe('Cancel work order', () => {
  beforeEach(() => {
    ...
    cy.fixture('repairs/work-order.json')
      .then((workOrder) => {
        workOrder.dateRaised = now.toISOString() // override fixture property
      })
      .as('workOrder')
  ...
  context(
    'When less than an hour has passed since work order date raised',
    () => {
      it('Cancel a work order', () => {
        const cancelTime = now.setMinutes(now.getMinutes() + 59)
        cy.clock(cancelTime)
            ...
            jobStatusUpdates: [
              {
                ...
                eventTime: '2021-01-18T17:26:20.000Z',
              },
            ],
          })
  ...
  context(
    'When more than an hour has passed since work order date raised',
    () => {
      it('Does not display cancel a work order', () => {
        const cancelTime = now.setMinutes(now.getMinutes() + 61)
        cy.clock(cancelTime)

        // Work order view page does not have a link to cancel work order
        cy.contains('Cancel Works Order').should('not.exist')

        // Directly navigate to the url
        cy.visit(`${Cypress.env('HOST')}/work-orders/10000012/cancel`)
        cy.contains('Cancel repair').should('not.exist')
      })
    }
  )
})
