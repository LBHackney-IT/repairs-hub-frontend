...
describe('Raise repair form', () => {
  beforeEach(() => {
    ...
    cy.intercept(
      { method: 'GET', path: '/api/properties/legalDisrepair/*' },
      { body: { propertyIsInLegalDisrepair: false } }
    ).as('legalDisrepairRequest')
  ...
  it('Submits work order task details to raise a work order', () => {
    ...
    cy.get('#repair-request-form').within(() => {
      ...
      cy.contains('+ Add another SOR code').click()
      cy.get('input[id="rateScheduleItems[3][code]"]').clear().type('DES')
      ...
      cy.intercept(
        {
          method: 'GET',
          path:
            '/api/schedule-of-rates/codes?tradeCode=PL&propertyReference=00012345&contractorReference=H01&isRaisable=true?q=DES',
        },
        {
          body: [
            {
  ...
  it('Submits an order above the user raise limit for authorisation', () => {
    ...
    cy.get('input[id="rateScheduleItems[1][code]"]')
      ...
      .then((datalist) => {
        ...
        cy.wrap(datalist).type('60020 - BATHROOM PLUMBING REPAIRS')
      })
    ...
    cy.get('input[id="rateScheduleItems[2][code]"]')
      ...
      .then((datalist) => {
        ...
        cy.wrap(datalist).type('60030 - KITCHEN PLUMBING REPAIRS')
      })
  ...
  describe('Legal Disrepair information', () => {
    ...
    context('When property is not in legal desrepair', () => {
      beforeEach(() => {
        cy.intercept(
          { method: 'GET', path: '/api/properties/legalDisrepair/00012345' },
          { body: { propertyIsInLegalDisrepair: false } }
