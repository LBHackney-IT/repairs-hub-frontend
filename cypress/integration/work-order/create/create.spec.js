...
describe('Raise repair form', () => {
  ...
  it.only('Validates missing form inputs', () => {
    ...
    // expect 'Please select a budget code'
  ...
  context('as an agent', () => {
    beforeEach(() => {
      ...
      // stub a request for the budget code endpoint
      // cy.intercept(
      //   {
      //     method: 'GET',
      //     // is property reference needed? Not sure
      //     path:
      //       '/api/budgetCodes?propertyReference=00012345&tradeCode=PL&contractorReference=BLAH',
      //   },
      //   { fixture: 'budgetCodes.json' }
    ...
    it('Submits work order task details to raise a work order', () => {
      ...
      cy.get('#repair-request-form').within(() => {
        ...
        // TODO: check there is no budget code input

        cy.wait('@sorCodesRequest') // need to include budget code in request???
      ...
      cy.wait('@apiCheck', { requestTimeout: 7000 }).then(({ request }) => {
        ...
        cy.wrap(request.body).should('deep.equal', {
          ...
          budgetCode: 'abc', // matches the default from API
        })
      })
  ...
  context('As a budget code officer', () => {
    beforeEach(() => {
      // need trades, contractor, budget codes, SORs + priority(?)
    })

    it('submits a raise repair form including my budget code selection', () => {
      // wait including trades
      // select trade
      // wait contractors request
      // select contractor
      // wait budget codes
      // check default budget code
      // select another budget code
    })
  })
